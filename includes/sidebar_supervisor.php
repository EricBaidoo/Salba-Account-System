...
<div class="fixed left-0 top-0 bottom-0 w-72 bg-white border-r border-gray-100 z-50 flex flex-col shadow-sm" id="sidebar">
    <div class="flex-shrink-0 flex items-center gap-3 px-5 py-5 border-b border-gray-100">
        ...
        <div>
            ...
            <div class="text-[10px] text-gray-400 font-medium uppercase tracking-wider">Supervisor Portal</div>
        </div>
    </div>
    <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-1">
        ...
        <a href="<?= nav_link($base_path, 'supervisor/dashboard.php') ?>" class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all <?= nav_active('dashboard.php', $current_page) ?>">
            <i class="fas fa-chart-line w-4 text-center text-blue-500"></i>
            <span class="text-sm">Supervisor Dashboard</span>
        </a>
        <div class="pt-4 pb-1 px-2 border-t border-gray-100 mt-2">
            <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest flex items-center gap-1.5"><i class="fas fa-book"></i> Academics Tracker</span>
        </div>
        <a href="<?= nav_link($base_path, 'supervisor/lesson_plans.php') ?>" class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all <?= nav_active('lesson_plans.php', $current_page) ?>">
        ...
        <a href="<?= nav_link($base_path, 'academics/report.php') ?>" class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all <?= nav_active('report.php', $current_page) ?>">
    ...
    <div class="p-4 border-t border-gray-100">
        <a href="<?= $root_path ?>logout.php" class="flex items-center gap-3 px-3 py-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors text-sm font-semibold">
