...
$att_res = $conn->prepare("SELECT s.class, COUNT(DISTINCT a.student_id) as marked_count FROM attendance a JOIN students s ON a.student_id = s.id WHERE a.attendance_date = ? AND s.status='active' GROUP BY s.class");
if ($att_res) {
...
if ($grad_res) {
    ...
    $grad_res->execute();
    $res = $grad_res->get_result();
    while ($row = $res->fetch_assoc()) {
        if (isset($class_progress[$row['class_name']])) {
            $class_progress[$row['class_name']]['graded_subjects'] = (int)$row['graded_count'];
        }
    }
    $grad_res->close();
}
                ...
                <table class="w-full text-left border-collapse text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        <?php if(empty($class_progress)): ?>
                            <tr><td colspan="4" class="px-6 py-8 text-center text-gray-400">No active classes found.</td></tr>
                        <?php else: foreach($class_progress as $cname => $data): 
                            ...
                            // Grades logic
                            $grd_total = $data['expected_subjects'];
                            $grd_marked = $data['graded_subjects'];
                            $grd_pct = $grd_total > 0 ? round(($grd_marked / $grd_total) * 100) : 0;
                            
                            $grd_color = 'bg-red-500';
                            if ($grd_pct == 100) $grd_color = 'bg-emerald-500';
                            elseif ($grd_pct > 0) $grd_color = 'bg-blue-500';
                        ?>
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-between text-xs mb-1">
                                    <span class="font-medium text-gray-700"><?php echo $att_marked; ?> of <?php echo $att_total; ?> Marked</span>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-between text-xs mb-1">
                                    <span class="font-medium text-gray-700"><?php echo $grd_marked; ?> of <?php echo $grd_total; ?> Subjects</span>
                                    <span class="font-bold <?php echo str_replace('bg-','text-',$grd_color); ?>"><?php echo $grd_pct; ?>%</span>
                                </div>
                                <div class="w-full bg-gray-100 rounded-full h-2">
                                    <div class="<?php echo $grd_color; ?> h-2 rounded-full transition-all duration-500" style="width: <?php echo $grd_pct; ?>%"></div>
                                </div>
                            </td>

                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="attendance.php?class=<?php echo urlencode($cname); ?>&action=entry" class="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center text-gray-500 hover:bg-green-100 hover:text-green-600 transition" title="Mark Attendance">
                                        <i class="fas fa-calendar-check text-sm"></i>
                                    </a>
                                    <a href="grades.php?class=<?php echo urlencode($cname); ?>&action=entry" class="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center text-gray-500 hover:bg-yellow-100 hover:text-yellow-600 transition" title="Enter Marks">
