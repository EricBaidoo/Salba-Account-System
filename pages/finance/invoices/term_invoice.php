...
/**
 * ARREARS HANDLING:
 * We now materialize previous semester arrears as a fee within the
 * current semester ("Arrears Carry Forward"). Invoices should therefore
 * list arrears inside the current semester fees rather than as a separate
 * computed block. Payments and balances are scoped by semester+year.
 */
...
// Show selection form if no semester specified
if (empty($semester)) {
    ...
    <head>
        <meta charset="UTF-8">
        <title>Generate Semester Invoices - Salba Montessori</title>
        <link href="https://cdn.tailwindcss.com" rel="stylesheet">
        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
            <link rel="stylesheet" href="../../../assets/css/style.css">
</head>
    <body class="clean-body">
        <div class="clean-max-w-7xl mx-auto">
            <div class="clean-header">
                <div class="clean-header-content">
                    ...
                    <div class="clean-header-title">
                        <h1><i class="fas fa-file-invoice"></i>Generate Semester Invoices</h1>
                        <p>Create prinw-full border-collapse invoices for all students</p>
                    </div>
                </div>
            </div>
                    ...
                    <form method="GET" action="term_invoice.php">
                        <div class="flex flex-wrap gap-3">
                            ...
                            <div class="col-md-4">
                                ...
                                <select class="clean-w-full px-3 py-2 border border-gray-300 rounded" id="academic_year" name="academic_year">
                            ...
                            <div class="col-md-4">
                                <label for="class" class="clean-block text-sm font-medium mb-">
                                    <i class="fas fa-school"></i>Filter by Class
                                </label>
                                <select class="clean-w-full px-3 py-2 border border-gray-300 rounded" id="class" name="class">
                        ...
                        <div class="clean-info-box mt-4">
                            ...
                            <span>This will generate invoices for all active students with fees assigned for the selected semester.</span>
                        </div>

                        <div class="text-center mt-4">
                            <button type="submit" class="clean-px-3 py-2 rounded-primary clean-px-3 py-2 rounded-lg">
...
foreach ($students as &$student) {
    ...
    // Fetch current semester fees (includes arrears carry-forward if any)
    $fees_sql = "
    ...
    // Arrears are now part of current semester fees; do not compute separately to avoid double counting
    $student['arrears'] = 0.0;
    
    // Get total paid for THIS semester/year only (usually zero at invoice time)
    ...
    // With arrears embedded in current semester fees, total bill is simply the semester total
...
?>
    ...
    <head>
        <meta charset="UTF-8">
        <title>Semester Invoices - <?php echo htmlspecialchars($semester); ?></title>
    ...
    <body class="invoice-body">
        <div class="no-print no-print-actions">
            <?php if (isset($_GET['generated']) && $_GET['generated'] == 1): ?>
                <div class="clean-success-box mb-">
            ...
            <div class="clean-action-bar">
                <div class="clean-action-info">
                    ...
                    <p>Semester: <?php echo htmlspecialchars($semester); ?> | Year: <?php echo htmlspecialchars($display_academic_year); ?> | Class: <?php echo $class_filter === 'all' ? 'All' : htmlspecialchars($class_filter); ?></p>
                </div>
                <div class="clean-action-buttons">
                    <button onclick="downloadPDF()" class="clean-px-3 py-2 rounded-success clean-px-3 py-2 rounded-lg">
                        <i class="fas fa-download"></i>Download PDF
                    </button>
                    <a href="generate_term_bills.php" class="clean-px-3 py-2 rounded-primary clean-px-3 py-2 rounded-lg">
        ...
        <div class="invoice-page">
            ...
            <div class="bill-title">SCHOOL FEES BILL - <?php echo strtoupper($semester); ?></div>

            <!-- Student Details (2 columns x 3 rows) -->
            <div class="student-details">
                <table class="student-details-w-full border-collapse">
                    <tr>
                        <td>
                    ...
                    <tr>
                        <td>
                            ...
                            <span><?php echo strtoupper(htmlspecialchars($semester)); ?></span>
                        </td>
                        <td>
            ...
            <table class="fee-w-full border-collapse">
                <thead>
                    <tr>
                        <th class="fee-description-col">FEE DESCRIPTION</th>
                        <th class="fee-amount-col">AMOUNT (GHâ‚µ)</th>
                    </tr>
                </thead>
                <tbody>
            ...
            <table class="totals-w-full border-collapse">
                <tr>
                    <td class="label">TOTAL:</td>
                    <td class="value">GHâ‚µ <?php echo number_format($student['total_bill'], 2); ?></td>
                </tr>
            </table>
            ...
            <div class="footer">
                ...
                <div class="payment-section">
                    <h3>PAYMENT PLAN</h3>
                    <table class="payment-plan-w-full border-collapse">
                        <thead>
                            <tr>
        ...
        <?php if (count($students) == 0): ?>
            <div class="invoice-page">
                <div class="alert alert-info text-center">
                    ...
                    No students found with fees assigned for <strong><?php echo htmlspecialchars($semester); ?></strong>
                    <?php if ($class_filter !== 'all'): ?>
                        in class <strong><?php echo htmlspecialchars($class_filter); ?></strong>
                    <?php endif; ?>.
                </div>
                <div class="text-center">
                    <a href="term_invoice.php" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">
        ...
        function downloadPDF() {
            ...
            // Build download URL
            let downloadUrl = 'download_term_invoice.php?semester=' + encodeURIComponent(semester) + 
