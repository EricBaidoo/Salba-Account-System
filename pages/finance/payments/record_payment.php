...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($academic_year === '') {
        echo "<div class='p-4 bg-red-100 text-red-700 rounded border border-red-200'>Please select an academic year for this payment.</div>";
        exit;
    }
    if ($amount <= 0) {
        echo "<div class='p-4 bg-red-100 text-red-700 rounded border border-red-200'>Payment amount must be greater than zero.</div>";
        exit;
    }
    ...
    if ($payment_mode === 'general') {
        ...
        if ($fee_id > 0) {
            ...
            if ($fee_check->num_rows === 0) {
                echo "<div class='p-4 bg-red-100 text-red-700 rounded border border-red-200'>Invalid fee category selected.</div>";
                $fee_check->close();
                exit;
            }
            $fee_check->close();
        ...
        if ($student_id <= 0) {
            echo "<div class='p-4 bg-red-100 text-red-700 rounded border border-red-200'>No student selected.</div>";
            exit;
        }
...
<body>
    <div class="max-w-7xl mx-auto mt-5">
        <a href="record_payment_form.php" class="px-4 py-2 bg-gray-600 text-white rounded mb-">Back to Payment Form</a>
        <a href="view_payments.php" class="px-3 py-2 rounded px-3 py-2 rounded-info mb-">View All Payments</a>
    </div>
    </body>
</html>
