...
if ($student_id <= 0 || $term === '') {
    echo "<div class='p-4 bg-red-100 text-red-700 rounded border border-red-200'>Missing student_id or term.</div>";
    exit;
}
...
try {
    ...
    $pstmt->close();
    $conn->commit();
    echo "<div class='p-4 bg-green-100 text-green-700 rounded border border-green-200'>Reallocation completed. <a href='student_balance_details.php?id=" . $student_id . "&term=" . urlencode($term) . "&academic_year=" . urlencode($academic_year) . "&debug=1'>Back to details</a></div>";
} catch (Exception $e) {
    $conn->rollback();
