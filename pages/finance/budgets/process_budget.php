...
if ($action === 'create') {
    ...
    if (!$category || $amount <= 0 || !$semester || !$academic_year || !$start_date || !$end_date) {
        header('Location: add_budget_form.php?error=Invalid data');
        exit;
    }

    $stmt = $conn->prepare("INSERT INTO budgets (category, description, amount, semester, academic_year, start_date, end_date, alert_threshold, created_at) 
                           VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())");
    $stmt->bind_param('ssdsssi', $category, $description, $amount, $semester, $academic_year, $start_date, $end_date, $alert_threshold);
