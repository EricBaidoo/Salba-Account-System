...
<head>
    ...
    <style>
        ...
        .budget-card:focus-within { border-color: #6366f1; box-shadow: 0 0 0 0.25rem rgba(99, 102, 241, 0.1); }
    </style>
</head>
<body class="text-slate-900 leading-relaxed">
    ...
    <main class="admin-main-content lg:ml-72 p-4 md:p-8 p-10 pb-40 min-h-screen">
        ...
        <header class="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
            ...
            <a href="semester_budget.php?semester=<?= urlencode($current_term) ?>&academic_year=<?= urlencode($academic_year) ?>" class="bg-white border border-slate-200 text-slate-400 font-black text-[0.625rem] uppercase tracking-widest px-8 py-4 rounded-2xl hover:text-slate-600 hover:bg-slate-50 transition-all leading-none">
        ...
        <form id="budgetForm" action="process_semester_budget.php" method="POST" class="space-y-16">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
                ...
                <section class="space-y-10">
                    <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                        <h3 class="text-[0.6875rem] font-black text-slate-400 uppercase tracking-[0.3em] flex items-center gap-3">
                            <i class="fas fa-coins text-emerald-500"></i> Revenue Infrastructure
                        </h3>
                        <span class="text-[0.5625rem] font-black text-emerald-600 uppercase tracking-widest bg-emerald-50 px-3 py-1 rounded-full border border-emerald-100">Inbound Flow</span>
                    </div>
                        ...
                        <div class="budget-card bg-white rounded-[2rem] p-8 border border-slate-100 shadow-sm">
                            <div class="flex justify-between items-start mb-6">
                                <div>
                                    ...
                                    <p class="text-[0.5625rem] font-bold text-slate-400 uppercase mt-1">Institutional Realized: ₵<?= number_format($assigned_total, 2) ?></p>
                                </div>
                            ...
                            <div class="relative">
                                ...
                                <span class="absolute right-6 top-1/2 -translate-y-1/2 text-[0.625rem] font-black text-slate-300 uppercase">Projected</span>
                ...
                <section class="space-y-10">
                    <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                        ...
                        <span class="text-[0.5625rem] font-black text-rose-600 uppercase tracking-widest bg-rose-50 px-3 py-1 rounded-full border border-rose-100">Outbound Thresholds</span>
                    </div>
                            ...
                            <div class="flex justify-between items-start mb-6">
                                <div>
                                    ...
                                    <?php if($previous_term): ?>
                                        <p class="text-[0.5625rem] font-bold text-slate-400 uppercase mt-1">Previous Audit: ₵<?= number_format(getSemesterCategorySpending($conn, $cat['name'], $previous_term, $previous_academic_year), 2) ?></p>
                                    <?php endif; ?>
                                </div>
                            </div>
                            <div class="relative">
                                ...
                                <span class="absolute right-6 top-1/2 -translate-y-1/2 text-[0.625rem] font-black text-slate-300 uppercase">Cap</span>
            ...
            <div class="fixed bottom-10 left-80 right-10 sticky-summary rounded-[2.5rem] p-8 shadow-2xl shadow-indigo-900/40 z-40 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 pointer-events-auto">
                <div class="flex items-center gap-10">
                    <div>
                        <p class="text-[0.5625rem] font-black text-emerald-400 uppercase tracking-widest mb-1">Expected Revenue</p>
                        <h4 id="totalIncome" class="text-xl font-black text-white italic">₵0.00</h4>
                    </div>
                    <div class="w-[0.0625rem] h-10 bg-white/10"></div>
                    <div>
                        <p class="text-[0.5625rem] font-black text-rose-400 uppercase tracking-widest mb-1">Projected Spending</p>
                        <h4 id="totalExpenses" class="text-xl font-black text-white italic">₵0.00</h4>
                    </div>
                    <div class="w-[0.0625rem] h-10 bg-white/10"></div>
                    <div>
                        <p class="text-[0.5625rem] font-black text-indigo-300 uppercase tracking-widest mb-1">Fiscal Balance</p>
                        <h4 id="balance" class="text-xl font-black text-white italic uppercase">₵0.00</h4>
                    </div>
                </div>
                <div class="flex gap-4">
                    <button type="submit" class="bg-indigo-600 text-white font-black text-[0.625rem] uppercase tracking-widest px-10 py-5 rounded-2xl shadow-xl shadow-indigo-600/20 hover:bg-indigo-500 transition-all active:scale-95 leading-none">
                        Synchronize Projections
                    </button>
                    <a href="semester_budget.php" class="bg-white/5 border border-white/10 text-white font-black text-[0.625rem] uppercase tracking-widest px-8 py-5 rounded-2xl hover:bg-white/10 transition-all leading-none">
                        Cancel
                    </a>
                </div>
            </div>
