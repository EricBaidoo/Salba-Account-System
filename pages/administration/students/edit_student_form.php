...
// Ensure new columns exist in the student_parents table
$sp_columns_to_add = [
    'relationship' => 'VARCHAR(50) NULL',
    'is_primary' => 'TINYINT DEFAULT 0'
];
foreach ($sp_columns_to_add as $col => $type) {
    $check = $conn->query("SHOW COLUMNS FROM `student_parents` LIKE '$col'");
    if ($check && $check->num_rows === 0) {
        $conn->query("ALTER TABLE `student_parents` ADD COLUMN `$col` $type");
    }
}
...
if (empty($class_options)) {
    $class_options = ['Creche','Nursery 1','Nursery 2','KG 1','KG 2','Basic 1','Basic 2','Basic 3','Basic 4','Basic 5','Basic 6','Basic 7'];
}
...
$current_father_id = '';
if ($student) {
    $linked_parents_res = $conn->query("
        SELECT p.id, sp.relationship 
        FROM parents p
        JOIN student_parents sp ON p.id = sp.parent_id
        WHERE sp.student_id = $student_id AND sp.relationship IN ('Mother', 'Father')
    ");
    if ($linked_parents_res) {
        while ($row = $linked_parents_res->fetch_assoc()) {
            if ($row['relationship'] === 'Mother') {
                $current_mother_id = $row['id'];
            } elseif ($row['relationship'] === 'Father') {
                $current_father_id = $row['id'];
            }
        }
    }
}
...
<body class="bg-gray-50 text-gray-800">
    ...
    <main class="admin-main-content lg:ml-72 min-h-screen">
        ...
        <div class="bg-white border-b border-gray-100 px-8 py-6">
            ...
            <div>
                <h1 class="text-3xl font-extrabold text-gray-900 flex items-center gap-3">
                    <i class="fas fa-user-edit text-indigo-600"></i> Edit Student Profile
                </h1>
                <p class="text-gray-500 mt-2 font-medium">
                    Modify existing student records, residency settings, and parent linkages.
                </p>
            </div>
        </div>
        ...
        <div class="p-8 max-w-6xl mx-auto">
            ...
            <?php else: ?>
                ...
                <form action="update_student" method="POST" enctype="multipart/form-data" class="space-y-6">
                            ...
                            <div class="flex flex-col md:flex-row gap-8">
                                ...
                                <div class="flex-shrink-0">
                                    ...
                                    <label for="photo_input" class="photo-upload group cursor-pointer" id="photo_label">
                                        <img id="photo_preview" class="photo-preview <?= empty($student['photo_path']) ? 'hidden' : '' ?>" src="<?= !empty($student['photo_path']) ? '../../../' . htmlspecialchars($student['photo_path']) : '' ?>">
                                        <div id="photo_placeholder" class="flex flex-col items-center justify-center text-center p-2 <?= !empty($student['photo_path']) ? 'hidden' : '' ?>">
                                ...
                                <div class="flex-1 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                                    <div>
                                        ...
                                        <input type="text" name="first_name" class="field-input" value="<?= htmlspecialchars($student['first_name']) ?>" required>
                                    </div>
                                    <div>
                                        <label class="field-label">Last Name <span class="text-red-500">*</span></label>
                                        <input type="text" name="last_name" class="field-input" value="<?= htmlspecialchars($student['last_name']) ?>" required>
                                    </div>
                                    <div>
                                        <label class="field-label">Date of Birth</label>
                                        <input type="date" name="date_of_birth" class="field-input" value="<?= htmlspecialchars($student['date_of_birth'] ?? '') ?>">
                                    </div>
                                    <div class="lg:col-span-2">
                                        <label class="field-label">Place of Birth</label>
                                        <input type="text" name="place_of_birth" class="field-input" value="<?= htmlspecialchars($student['place_of_birth'] ?? '') ?>" placeholder="City or Hospital of birth">
                                    </div>
                                    <div>
                                        <label class="field-label">Account Status <span class="text-red-500">*</span></label>
                            ...
                            <div>
                                ...
                                <select name="class" class="field-input" required>
                                    ...
                                    <?php foreach($class_options as $c): ?>
                                        <option value="<?= htmlspecialchars($c) ?>" <?= $student['class'] === $c ? 'selected' : '' ?>><?= htmlspecialchars($c) ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            <div>
                                <label class="field-label">Date Admitted</label>
                                <input type="date" name="date_admitted" class="field-input" value="<?= htmlspecialchars($student['date_admitted'] ?? '') ?>">
                            </div>
                            <div>
                                <label class="field-label">Previous School (if any)</label>
                                <input type="text" name="previous_school" class="field-input" value="<?= htmlspecialchars($student['previous_school'] ?? '') ?>" placeholder="Name of previous institution">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="section-card overflow-hidden">
                        ...
                        <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div class="md:col-span-2">
                                ...
                                <textarea name="address" rows="2" class="field-input" placeholder="Full residential address" style="resize:none;"><?= htmlspecialchars($student['address'] ?? '') ?></textarea>
                            </div>
                            <div>
                                <label class="field-label">Landmark (Famous Location)</label>
                                <input type="text" name="landmark" class="field-input" value="<?= htmlspecialchars($student['landmark'] ?? '') ?>" placeholder="Nearest landmark to residence">
                            </div>
                            <div>
                                <label class="field-label">Emergency Contact</label>
                                <input type="tel" name="emergency_contact" class="field-input" value="<?= htmlspecialchars($student['emergency_contact'] ?? '') ?>" placeholder="Name and Phone Number">
                            </div>
                        </div>
                    </div>
                                ...
                                <div>
                                    ...
                                    <select name="existing_mother_id" class="field-input bg-purple-50 border-purple-200 focus:border-purple-500" onchange="toggleMotherFields(this.value)">
                                        ...
                                        <?php foreach($parents_list as $p): ?>
                                            <option value="<?= $p['id'] ?>" <?= $p['id'] == $current_mother_id ? 'selected' : '' ?>><?= htmlspecialchars(trim($p['title'].' '.$p['first_name'].' '.$p['last_name'])) ?> (<?= htmlspecialchars($p['phone']) ?>)</option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
                            ...
                            <div class="bg-indigo-50 p-5 rounded-xl border border-indigo-100 space-y-4">
                                ...
                                <div>
                                    ...
                                    <select name="existing_father_id" class="field-input bg-indigo-50 border-indigo-200 focus:border-indigo-500" onchange="toggleFatherFields(this.value)">
                                        ...
                                        <?php foreach($parents_list as $p): ?>
                                            <option value="<?= $p['id'] ?>" <?= $p['id'] == $current_father_id ? 'selected' : '' ?>><?= htmlspecialchars(trim($p['title'].' '.$p['first_name'].' '.$p['last_name'])) ?> (<?= htmlspecialchars($p['phone']) ?>)</option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
    ...
    <script>
        ...
        window.addEventListener('DOMContentLoaded', () => {
            const mSelect = document.querySelector('select[name="existing_mother_id"]');
            const fSelect = document.querySelector('select[name="existing_father_id"]');
            if (mSelect) toggleMotherFields(mSelect.value);
            if (fSelect) toggleFatherFields(fSelect.value);
        });
    </script>
</body>
</html>
